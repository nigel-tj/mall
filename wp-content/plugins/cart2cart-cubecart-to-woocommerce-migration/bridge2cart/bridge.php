  ...
  public function magentoClearCache()
  {
    ...
    if ($phpExecutable) {
      ...
      if (file_exists(M1_STORE_BASE_DIR . 'app/etc/env.php')) {
        ...
        @exec("nohup $indexer > /dev/null 2>/dev/null & echo $!");
        @exec("nohup $imagesResize > /dev/null 2>/dev/null & echo $!");
        @exec("nohup $clearCache > /dev/null 2>/dev/null & echo $!");
...
class M1_Bridge
{
  ...
  public function run()
  {
    $actionIgnoreSelfTest = array('update' => true, 'move' => true, 'getserverip' => true);
    $checkDbConnectionActions = array('carttype' => true, 'clearcache' => true, 'mysqlver' => true, 'phpinfo' => true, 'query' => true, 'querymultiple' => true);
